...
interface User {
    ...
    lastName: string;
    email?: string;
    // add other user properties as needed from your API response
    name?: string;
    avatar?: string | null;
}
...
interface PersonFilterPopoverProps {
    ...
    open: boolean;
    onClose: () => void;
    onApplyFilter: (selectedUsers: string[], filterType: 'assignedTo' | 'createdBy' | 'both') => void;
    currentFilters?: {
...
export default function PersonFilterPopover({
    ...
    const [filterType, setFilterType] = useState<'assignedTo' | 'createdBy' | 'both'>('both');

    // Initialize selected users from current filters
    useEffect(() => {
        if (currentFilters.assignedToIds && currentFilters.createdByIds) {
            // If both filters are set
            const combinedUsers = Array.from(new Set([...currentFilters.assignedToIds, ...currentFilters.createdByIds]));
            setSelectedUsers(combinedUsers);
            setFilterType('both');
        } else if (currentFilters.assignedToIds) {
            setSelectedUsers(currentFilters.assignedToIds);
            setFilterType('assignedTo');
        } else if (currentFilters.createdByIds) {
            setSelectedUsers(currentFilters.createdByIds);
            setFilterType('createdBy');
        }
    }, [currentFilters, open]);

    const users = useMemo(() => {
        return (companyUsersData as CompanyUsersResponse)?.users || [];
    }, [companyUsersData]);
                ...
                <Box sx={{ mb: 2 }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                        <FormControlLabel
                            control={
                                <Checkbox
                                    checked={filterType === 'assignedTo'}
                                    onChange={() => setFilterType('assignedTo')}
                                    size="small"
                                />
                            }
                            label="Assigned To"
                        ...
                        <FormControlLabel
                            control={
                                <Checkbox
                                    checked={filterType === 'createdBy'}
                                    onChange={() => setFilterType('createdBy')}
                                    size="small"
                                />
                            }
                            label="Created By"
                        ...
                        <FormControlLabel
                            ...
                            label="Both (Assigned To OR Created By)"
                        />
                    </Box>
                </Box>
