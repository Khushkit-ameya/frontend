...
type ColorPickerProps = {
  value?: string; // hex string, with or without '#'
  default_value?: string; // initial value when uncontrolled
  onChange?: (hex: string) => void; // hex with '#'
  className?: string;
};

const ColorPicker: React.FC<ColorPickerProps> = ({ value, default_value = "#1C9488", onChange, className }) => {
  // Initialize from controlled prop or a default
  const [color, setColor] = useState<Color>(() => {
    const initialHex = sanitizeHex((value ?? default_value) || '#000000');
    const hsl = hexToHsl({ hex: initialHex });
    return { ...hsl, hex: sanitizeHex(initialHex) };
  });

  // Sync internal state when controlled value changes
  useEffect(() => {
    if (value) {
      const v = sanitizeHex(value);
      const hsl = hexToHsl({ hex: v });
      setColor({ ...hsl, hex: v });
    }
  }, [value]);
  // Update from hex input
  const emit = (nextHexNoHash: string, nextH?: number, nextS?: number, nextL?: number) => {
    const hexWithHash = `#${nextHexNoHash}`;
    onChange?.(hexWithHash);
    // If uncontrolled, keep internal state; if controlled, rely on parent
  };
  ...
  const handleHexInputChange = (newVal: string) => {
    ...
    if (hex.length === 6) {
      ...
      setColor((prev) => ({ ...prev, ...hsl, hex }));
      emit(hex, hsl.h, hsl.s, hsl.l);
  ...
  return (
    <>
      ...
      <div
        ...
        className={clsx("z-30 flex w-full max-w-[300px] select-none flex-col items-center gap-3 overscroll-none rounded-2xl border border-zinc-200 bg-white p-4 shadow-md dark:border-zinc-700 dark:bg-zinc-900", className)}
      >
        <DraggableColorCanvas
          ...
          handleChange={(parital) => {
            setColor((prev) => {
              ...
              const next = { ...value, hex: hex_formatted };
              emit(hex_formatted, next.h, next.s, next.l);
              return next;
            });
          }}
        />
        <input
          ...
          onChange={(e) => {
            ...
            setColor((prev) => {
              ...
              const next = { ...rest, hex: hex_formatted } as Color;
              emit(hex_formatted, next.h, next.s, next.l);
              return next;
            });
          }}
