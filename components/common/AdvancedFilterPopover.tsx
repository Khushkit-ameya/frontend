...
// Task field definitions with their types and available operators
const TASK_FIELDS = [
  // Task-level fields
  { key: 'taskId', label: 'Task ID', type: 'string', category: 'task' },
  { key: 'taskName', label: 'Task Name', type: 'string', category: 'task' },
  { key: 'description', label: 'Task Description', type: 'string', category: 'task' },
  { key: 'status', label: 'Task Status', type: 'string', hasOptions: true, category: 'task' },
  { key: 'priority', label: 'Task Priority', type: 'string', hasOptions: true, category: 'task' },
  { key: 'taskType', label: 'Task Type', type: 'string', category: 'task' },
  { key: 'tags', label: 'Task Tags', type: 'string[]', category: 'task' },
  { key: 'startDate', label: 'Task Start Date', type: 'date', category: 'task' },
  { key: 'endDate', label: 'Task End Date', type: 'date', category: 'task' },
  
  // Subtask-level fields (prefixed with subtask)
  { key: 'subtaskId', label: 'Subtask ID', type: 'string', category: 'subtask' },
  { key: 'subtaskName', label: 'Subtask Name', type: 'string', category: 'subtask' },
  { key: 'subtaskDescription', label: 'Subtask Description', type: 'string', category: 'subtask' },
  { key: 'subtaskStatus', label: 'Subtask Status', type: 'string', hasOptions: true, category: 'subtask' },
  { key: 'subtaskPriority', label: 'Subtask Priority', type: 'string', hasOptions: true, category: 'subtask' },
  { key: 'subtaskType', label: 'Subtask Type', type: 'string', category: 'subtask' },
  { key: 'subtaskTags', label: 'Subtask Tags', type: 'string[]', category: 'subtask' },
  { key: 'subtaskStartDate', label: 'Subtask Start Date', type: 'date', category: 'subtask' },
  { key: 'subtaskEndDate', label: 'Subtask End Date', type: 'date', category: 'subtask' },
];

// Project field definitions (for backward compatibility)
...
const FIELD_OPTIONS: Record<string, string[]> = {
  ...
  subtaskStatus: ['Pending', 'In Progress', 'Completed', 'On Hold', 'Cancelled'],
  subtaskPriority: ['Low', 'Medium', 'High', 'Critical'],
};
...
export interface FilterCondition {
  ...
  value: string;
  fieldType: string;
  category?: 'task' | 'subtask'; // Track whether this is a task or subtask filter
}
...
interface AdvancedFilterPopoverProps {
  ...
  title?: string;
  mode?: 'project' | 'task'; // Specify what type of filters to show
  onClearFilters?: () => void;
}
...
export default function AdvancedFilterPopover({
  ...
  const [filters, setFilters] = useState<FilterCondition[]>(() => {
    return initialFilters && initialFilters.length > 0 ? initialFilters : [createEmptyFilter()];
  });
  ...
  const FIELDS = mode === 'task' ? TASK_FIELDS : PROJECT_FIELDS;
  ...
  // Keep filters in sync with parent state when opening
  useEffect(() => {
    if (open) {
  ...
  // Ensure there's always at least one filter block rendered
  useEffect(() => {
    if (filters.length === 0) {
      setFilters([createEmptyFilter()]);
    }
  ...
  const handleFieldChange = (filterId: string, fieldKey: string) => {
    ...
    updateFilter(filterId, {
      ...
      value: '',
      category: (field as any).category || undefined
    });
  };
                    ...
                    <Stack spacing={2}>
                      ...
                      <Box sx={{ display: 'flex', gap: 1.5, width: '100%' }}>
                        ...
                        <FormControl size="small" sx={{ minWidth: 200 }}>
                          ...
                          >
                            {/* Group by category in task mode */}
                            {mode === 'task' ? (
                              [
                                <MenuItem key="task-header" disabled sx={{ fontWeight: 600, fontSize: '0.875rem', color: '#c81c1f' }}>
                                  Task Fields
                                </MenuItem>,
                                ...FIELDS.filter((f: any) => f.category === 'task').map(field => (
                                  <MenuItem 
                                    key={field.key} 
                                    value={field.key}
                                    sx={{
                                      ...
                                      '&:hover': {
                                        bgcolor:
                                          field.type === 'string' ? 'green.100' :
                                            field.type === 'number' ? '#fff0f2' :
                                              field.type === 'date' ? 'orange.100' :
                                                field.type.endsWith('[]') ? 'purple.100' :
                                                  'grey.100',
                                      }
                                    }}
                                  >
                                ...
                                <MenuItem key="subtask-header" disabled sx={{ fontWeight: 600, fontSize: '0.875rem', color: '#c81c1f', mt: 1 }}>
                                  Subtask Fields
                                </MenuItem>,
                                ...FIELDS.filter((f: any) => f.category === 'subtask').map(field => (
                                  <MenuItem 
                                    key={field.key} 
                                    value={field.key}
                                    sx={{
        ...
        <Box sx={{ p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Box>
            <Button
              ...
              size="small"
              disabled={filters.length === 0 || (filters.length === 1 && !filters[0].field)}
            >
              Clear All
            </Button>
          </Box>
